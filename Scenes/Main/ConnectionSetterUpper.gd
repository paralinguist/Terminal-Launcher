...
func _creating_url():
	...
	var client_settings_location
	
	if client_directory.open("clients") == OK:
		if file_check.file_exists("clients/client_settings.txt"):
			client_settings_location = "./clients/client_settings.txt"
		else:
			client_directory.copy("client_settings.txt.template", "client_settings.txt")
			client_settings_location = "./clients/client_settings.txt"
